...
const labels = ['Dirección virtual', 'Registro CR3', 'Page Directory', 'Page Table', 'Dirección física'];
...
$('#app').innerHTML = `
  <header class="site-header">
    ...
    <div class="header-inner">
      ...
      <nav aria-label="Navegación principal"><a href="#simulador" class="nav-active">Simulador</a><button data-dialog="guide">Guía rápida</button><button data-dialog="theory" aria-haspopup="dialog">Esquema teórico</button><button data-dialog="about">Sobre el proyecto ${icon('info')}</button></nav>
...
function nearbyRows(index) {
  const rows = new Set([0, ...Array.from({ length: 3 }, (_, i) => index + i - 1).filter(x => x >= 0 && x <= 1023), 1023]);
...
function tableRows(index, entries, enabled, tone) {
  return nearbyRows(index).map(row => row === null ? '<tr class="ellipsis"><td>⋮</td><td>⋮</td><td></td></tr>' : `<tr class="${row === index && enabled ? `selected ${tone}` : ''}"><td>${row === index && enabled ? '<span class="row-pointer">▸</span>' : ''}${row}</td><td>${hex(entries?.get(row) ?? 0)}</td><td>${row === index && enabled ? (entries?.get(row) & 1 ? '✓' : '!') : ''}</td></tr>`).join('');
}
...
function render() {
  ...
  const r = result;
  const fault = r.fault && step >= r.fault.stage;
  const reached = n => step >= n && !(r.fault && r.fault.stage < n);
  ...
  $('.stepper').querySelectorAll('button').forEach((button, i) => {
    ...
    button.disabled = Boolean(r.fault && i > r.fault.stage);
  ...
  $('#scene').innerHTML = `
    <div class="address-breakdown"><div class="address-source"><span>DIRECCIÓN VIRTUAL</span><strong>${hex(r.address)}</strong></div><span class="split-arrow">=</span><div class="bit-segment blue"><span>Directorio <small>10 bits</small></span><strong>${r.pdi}</strong><code>${bin.slice(0, 10)}</code></div><div class="bit-segment green"><span>Tabla <small>10 bits</small></span><strong>${r.pti}</strong><code>${bin.slice(10, 20)}</code></div><div class="bit-segment orange"><span>Offset <small>12 bits</small></span><strong>${hex(r.offset, 3)}</strong><code>${bin.slice(20)}</code></div><div class="offset-note">Este viaja<br>sin cambios <span>↴</span></div></div>
    <div class="journey">
      <article class="cpu-node ${step <= 1 ? 'node-active' : ''}"><span class="node-eyebrow">EL PUNTO DE PARTIDA</span>${cpuIllustration()}<h3>Todo empieza en la CPU</h3><p>La MMU hace la traducción.</p><div class="cr3-box ${step === 1 ? 'focus-ring' : ''}"><span>CR3</span><code>${reached(1) ? hex(CR3) : '¿Dónde empieza?'}</code></div><span class="node-footnote">Base física del directorio</span></article>
      <div class="connector ${reached(1) ? 'lit blue-line' : ''}" aria-hidden="true"><span>${reached(1) ? 'CR3' : ''}</span><svg viewBox="0 0 45 28"><path d="M1 9h26V2l16 12-16 12v-7H1z"/></svg></div>
      <article class="table-node ${step === 2 ? 'node-active' : ''}"><div class="node-title blue"><span class="node-icon">▤</span><h3>Page Directory<small>Directorio de páginas</small></h3><span class="node-index">01</span></div><div class="table-base">Base <code>${reached(1) ? hex(r.directoryBase) : '— — —'}</code></div><table aria-label="Entradas del directorio de páginas"><thead><tr><th scope="col">Índice</th><th scope="col">Entrada · PDE</th><th scope="col"><span class="sr-only">Estado</span></th></tr></thead><tbody>${tableRows(r.pdi, memory.directory, reached(2), 'blue')}</tbody></table><div class="table-bottom">1.024 entradas <span>× 4 bytes</span></div><div class="lookup-label blue-label">${reached(2) ? `Índice del directorio = <strong>${r.pdi}</strong>` : 'Los primeros 10 bits eligen la entrada'}</div></article>
      <div class="connector ${reached(2) && !fault ? 'lit green-line' : ''}" aria-hidden="true"><span>${reached(2) && !fault ? 'PDE' : ''}</span><svg viewBox="0 0 45 28"><path d="M1 9h26V2l16 12-16 12v-7H1z"/></svg></div>
      <article class="table-node ${step === 3 ? 'node-active' : ''} ${!reached(2) || r.fault?.stage === 2 ? 'pending' : ''}"><div class="node-title green"><span class="node-icon">▤</span><h3>Page Table<small>Tabla de páginas</small></h3><span class="node-index">02</span></div><div class="table-base">Base <code>${reached(2) && r.fault?.stage !== 2 ? hex(r.tableBase) : '— — —'}</code></div><table aria-label="Entradas de la tabla de páginas"><thead><tr><th scope="col">Índice</th><th scope="col">Entrada · PTE</th><th scope="col"><span class="sr-only">Estado</span></th></tr></thead><tbody>${tableRows(r.pti, reached(2) && r.fault?.stage !== 2 ? memory.tables.get(r.tableBase) : null, reached(3), 'green')}</tbody></table><div class="table-bottom">1.024 entradas <span>× 4 bytes</span></div><div class="lookup-label green-label">${reached(3) ? `Índice de la tabla = <strong>${r.pti}</strong>` : 'Los siguientes 10 bits eligen la entrada'}</div></article>
      <div class="connector ${reached(3) && !fault ? 'lit orange-line' : ''}" aria-hidden="true"><span>${reached(3) && !fault ? 'PTE' : ''}</span><svg viewBox="0 0 45 28"><path d="M1 9h26V2l16 12-16 12v-7H1z"/></svg></div>
      <article class="physical-node ${step === 4 ? 'node-active' : ''} ${!reached(3) || fault ? 'pending' : ''}"><div class="physical-heading"><span class="node-icon">▥</span><h3>Memoria física<small>El destino, en la RAM</small></h3></div><div class="memory-stack"><div class="memory-top">⋮</div><div class="memory-frame frame-purple"></div><div class="memory-frame frame-blue"></div><div class="memory-frame frame-active"><span>Marco físico</span><code>${reached(3) && !fault ? hex(r.frameBase) : '— — —'}</code>${step === 4 ? byteMarker(r) : ''}</div><div class="memory-frame frame-orange"></div><div class="memory-frame frame-pink"></div><div class="memory-bottom">⋮</div><span class="memory-bracket">4 KiB</span></div><div class="physical-result ${step === 4 ? 'arrived' : ''}"><span>${step === 4 ? '¡BYTE ENCONTRADO!' : 'DIRECCIÓN FÍSICA'}</span><strong>${step === 4 ? hex(r.physical) : '¿Cuál será?'}</strong></div></article>
    </div>`;
  const explanations = [
    ['SEPARAMOS LOS 32 BITS', 'Una dirección, tres pistas.', `Los primeros 10 bits eligen el directorio (${r.pdi}); los siguientes 10, la tabla (${r.pti}). Los últimos 12 indican el desplazamiento: ${r.offset} bytes.`, '10 + 10 + 12'],
    ['BUSCAMOS EL DIRECTORIO', 'CR3 nos da el punto de partida.', `La MMU lee CR3. Su base ${hex(CR3)} es la dirección física del Page Directory del proceso. El directorio también está en RAM.`, '¡Por acá!'],
    ['LEEMOS LA ENTRADA PDE', `La entrada ${r.pdi} señala una tabla.`, `La MMU lee ${hex(r.directoryBase)} + ${r.pdi} × 4 = ${hex(r.pdeAddress)}. La PDE vale ${hex(r.pde)}; al quitar los flags, la base de la tabla es ${hex(r.tableBase)}.`, `PDI = ${r.pdi}`],
    ['LEEMOS LA ENTRADA PTE', `La entrada ${r.pti} señala un marco.`, `La MMU lee ${hex(r.tableBase)} + ${r.pti} × 4 = ${hex(r.pteAddress)}. La PTE vale ${hex(r.pte)}; su base de marco es ${hex(r.frameBase)}. P, R/W y U/S permiten este acceso.`, `PTI = ${r.pti}`],
...
function advance() {
  if (!result) return;
  if (step < 4 && !(result.fault && step >= result.fault.stage)) step++;
  if (step === 4 || result.fault && step >= result.fault.stage) stop();
  render();
}
...
function play() {
  ...
  if (step === 4 || result.fault && step >= result.fault.stage) step = 0;
