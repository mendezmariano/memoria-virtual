...
function render() {
  ...
  $('#scene').innerHTML = `${addressMarkup}<div class="journey journey-progressive journey-stage-${Math.min(step, 3)}"><article class="cpu-node ${step === 0 ? 'node-active component-enter' : ''}"><span class="node-eyebrow">EL PUNTO DE PARTIDA</span>${cpuIllustration()}<h3>Todo empieza en la CPU</h3><p>La MMU hace la traducción.</p><div class="cr3-box ${step === 0 ? 'focus-ring' : ''}"><span>CR3</span><code>${hex(CR3)}</code></div><span class="node-footnote">Base física del directorio</span></article>${directoryMarkup}${tableMarkup}${physicalMarkup}</div>`;
  const explanations = [
